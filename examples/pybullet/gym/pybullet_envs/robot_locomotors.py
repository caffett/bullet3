...
class Hopper(WalkerBase):
    ...
    def __init__(self):
        WalkerBase.__init__(self, "hopper.xml", "torso",
                            action_dim=3, obs_dim=15, power=0.75)
...
class Walker2D(WalkerBase):
    ...
    def __init__(self):
        WalkerBase.__init__(self, "walker2d.xml", "torso",
    ...
    def safety_reward(self, z, pitch):
        # (z-0.8) + (1.0-abs(pitch)) if z > 0.8 and abs(pitch) < 1.0 else -99999
        return 1
...
class HalfCheetah(WalkerBase):
    ...
    def __init__(self):
        WalkerBase.__init__(self, "half_cheetah.xml", "torso",
                            action_dim=6, obs_dim=26, power=0.90)

    def alive_bonus(self, z, pitch):
        # Use contact other than feet to terminate episode: due to a lot of
        # strange walks using knees
        return +1 if np.abs(pitch) < np.pi/2 else -1 
...
class Ant(WalkerBase):
    ...
    def __init__(self):
        WalkerBase.__init__(self, "ant.xml", "torso",
                            action_dim=8, obs_dim=28, power=2.5)
...
class Humanoid(WalkerBase):
    ...
    def __init__(self):
        WalkerBase.__init__(self,
                            ...
                            obs_dim=44,
                            power=0.41)
        # 17 joints, 4 of them important for walking (hip, knee), others may as
        # well be turned off, 17/4 = 4.25
    ...
    def alive_bonus(self, z, pitch):
        # 2 here because 17 joints produce a lot of electricity cost just from
        # policy noise, living must be better than dying
...
def get_cube(_p, x, y, z):
    body = _p.loadURDF(os.path.join(
        pybullet_data.getDataPath(), "cube_small.urdf"), [x, y, z])
...
def get_sphere(_p, x, y, z):
    body = _p.loadURDF(os.path.join(
        pybullet_data.getDataPath(), "sphere2red_nocol.urdf"), [x, y, z])
    part_name, _ = _p.getBodyInfo(body)
    part_name = part_name.decode("utf8")
    bodies = [body]
    return BodyPart(_p, part_name, bodies, 0, -1)
    ...
    def flag_reposition(self):
        ...
        if (self.flag):
            ...
            self.flag = get_sphere(
                self._p, self.walk_target_x, self.walk_target_y, 0.7)
...
class HumanoidFlagrunHarder(HumanoidFlagrun):
    ...
    def alive_bonus(self, z, pitch):
        if self.frame % 30 == 0 and self.frame > 100 and self.on_ground_frame_counter == 0:
            ...
            position = [
                target_xyz[0] + from_dist *
                np.cos(angle), target_xyz[1] + from_dist * np.sin(angle),
                target_xyz[2] + 1.0
            ]
            attack_speed_vector = target_xyz - np.array(position)
            attack_speed_vector *= attack_speed / \
                np.linalg.norm(attack_speed_vector)
            attack_speed_vector += self.np_random.uniform(
                low=-1.0, high=+1.0, size=(3,))
            self.aggressive_cube.reset_position(position)
            self.aggressive_cube.reset_velocity(
                linearVelocity=attack_speed_vector)
        if z < 0.8:
            self.on_ground_frame_counter += 1
        elif self.on_ground_frame_counter > 0:
            self.on_ground_frame_counter -= 1
        # End episode if the robot can't get up in 170 frames, to save
        # computation and decorrelate observations.
        self.frame += 1
    ...
    def potential_leak(self):
        # 0.00 .. 0.8 .. 1.05 normal walk, 1.2 when jumping
        z = self.body_xyz[2]
        z = np.clip(z, 0, 0.8)
    ...
    def calc_potential(self):
        ...
        #
        # Why robot whould stand up: to receive 100 points in potential field
        # difference.
